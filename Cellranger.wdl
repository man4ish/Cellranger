...
# Task to process raw sequencing data using Cell Ranger
task count {
    input {
        # Optional parameters
        ...
        # Required parameters
        String sid               # Sample ID for Cell Ranger
    ...
    command {
        ...
        cmd="count --id=${sid} --fastqs=$fastq_path --transcriptome=$ref_path"
        
        # Add optional parameters to the command if specified
        if ${nosecondary}; then cmd+=" --nosecondary"; fi
        if [ -n "${localcores}" ]; then cmd+=" --localcores"; cmd+=" ${localcores}"; fi
        if [ -n "${localmem}" ]; then cmd+=" --localmem"; cmd+=" ${localmem}"; fi
        if [ -n "${localvmem}" ]; then cmd+=" --localvmem"; cmd+=" ${localvmem}"; fi
        echo $cmd
...
# Task to aggregate results from multiple Cell Ranger outputs
task aggr {
    input {
        # Required parameters
        String aggr_id           # Aggregation ID for Cell Ranger
        File data_tar            # Tarball of archive files for molecule h5
    ...
    parameter_meta {
        aggr_id: "aggr id"
        data_tar: "archive files for molecule h5"
    ...
    command {
        ...
        # Construct Cell Ranger aggregation command
        ...
        # Run the Cell Ranger aggregation command
        /software/reboot-utils/cellranger/bin/cellranger $cmd
...
# Workflow to execute the Cell Ranger count and aggregation tasks
workflow Cellranger {
    input {
